...
import { parseSwaggerSchema, getMethodColor, getStatusColor } from '../utils/swaggerParser';
import type { SwaggerSchema, ParsedSwagger, SwaggerEndpoint } from '../types';

interface SwaggerViewerProps {
  data: SwaggerSchema | null;
}

export default function SwaggerViewer({ data }: SwaggerViewerProps) {
  if (!data) return null;

  const schema: ParsedSwagger | null = parseSwaggerSchema(data);
      ...
      <Box sx={{ mt: 3 }}>
        ...
        {Object.entries(schema.paths).map(([path, methods]) =>
            Object.entries(methods).map(([method, endpoint]) => {
              const details = endpoint as SwaggerEndpoint;
